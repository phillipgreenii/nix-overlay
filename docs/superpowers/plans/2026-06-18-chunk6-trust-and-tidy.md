...
- **Use the Edit tool for surgical changes to existing files.** Use Write only for the brand-new `verify-provenance.sh` and the test fixtures (none required here). `_sources/generated.nix` and `_sources/nvfetcher.json` are *generated* by `nvfetcher` — do not Write them by hand.
