...
| `cmux` | aarch64-darwin | [manaflow-ai/cmux](https://github.com/manaflow-ai/cmux) |
...
Note: the file uses a fenced code block containing a fenced code block. GFM handles nested fences if the outer fence uses more backticks than the inner — fine here since the outer document is rendered by GitHub's README handler which understands single fences with `nix` and the inner one is inside a quoted example. **Verification:** the implementer should preview the rendered output (`gh repo view --json description` is not enough; just visually inspect after pushing).
...
### Component 4 — Delete the stale `update-c9watch` step file
...
Note that after Component 2's gitignore addition, the *remaining* files in `.update-locks/steps/` (`bat-gherkin-syntax`, `nix-flake-update`, `tmux-mouse-swipe`, `tmux-nerd-font-window-name`, `tmux-open-nvim`, `update-beads-web`, `update-cmux`, `update-gascity`) are still tracked but newly untracked changes won't show. To purge them too:

```bash
git rm -rf --cached .update-locks/steps/
```

This removes all of them from the index. They remain on disk (still readable by `ul_run_step`) but git no longer tracks them. The next `update-locks.sh` run will update them locally but won't trigger a git diff.
